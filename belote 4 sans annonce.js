...
function gains(a){
    ...
    else if (a[0]<a[1] & a[4]==1 & a[1] != 252){
        partie = [a[2],162 + a[3],0];
        gagnants.push(-1);
    }
    else if(a[0]<a[1] & a[4]==1 & a[1] == 252){
        partie = [a[2],252 + a[3],0];
        gagnants.push(-1);

    }
    else if (a[0]>a[1] & a[4]==-1 & a[0] != 252){
        partie = [162 + a[2], a[3],0];
        gagnants.push(1)
    }
    else if (a[0]>a[1] & a[4]==-1 & a[0] == 252){
        partie = [252 + a[2], a[3],0];
        gagnants.push(1)
    }
    }
    if ((a[0]==a[1])&(a[0]!=0)){
...
function litigetotal3(){
    if (nombreParties >= 2){
        ...
        for (j=0; j< nombreParties-2; j = j+1){
            console.log("gerer litige partie n°",nombreParties-j);
            gererLitige(nombreParties-j);
...
function gererLitige(numero){
    ...
    if (nb >= 1){
        ...
        //console.log("preneurs ->",preneurs);
        //console.log("gagnants -->",gagnants);
        console.log("nb ->",nb,"historique ",historique);
    }
    calculSommesPartielles(nombreParties-1);
    afficherHistoriqueScores();
