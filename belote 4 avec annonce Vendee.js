...
function acterDernierePartie(){
    ...
    pointsAnnoncesStr = document.getElementById('listedannonces').value ;
    pointsAnnonces = parseInt(pointsAnnoncesStr);
    quiAnnoncesStr = document.getElementById('equipequialannonce').value;
    quiAnnonces = parseInt(quiAnnoncesStr);
    declarationAnnonces();
    console.log("points annonce ->",pointsAnnonces,"à qui -->",quiAnnoncesStr);
    annonces.push([0,0]);
    if (quiAnnonces == 1){
        annonces[nombreParties-1][0]= pointsAnnonces ;
    }
    if (quiAnnonces ==-1){
        annonces[nombreParties-1][1]= pointsAnnonces
    }
    ...
    if (nombrePointsNOUSStr != "" & nombrePointsEUXStr == ""){
        nombrePointsEUX = 162 - nombrePointsNOUS
    }
    if (nombrePointsNOUSStr == "" & nombrePointsEUXStr != ""){
        nombrePointsNOUS = 162 - nombrePointsEUX
    }
    if (nombrePointsNOUS <0 || nombrePointsNOUS > 162 || nombrePointsEUX <0 || nombrePointsEUX > 162){
        coeffPreneur = 0;
    }
    if ((nombrePointsNOUSStr == "" & nombrePointsEUXStr == "")||(nombrePointsNOUSStr != "" & nombrePointsEUXStr != "" & nombrePointsNOUS+nombrePointsEUX != 162)){
        coeffPreneur = 0 ;
    }
    resultatPriseNote =[nombrePointsNOUS,nombrePointsEUX,belote[nombreParties-1][0],belote[nombreParties-1][1],annonces[nombreParties-1][0],annonces[nombreParties-1][1],coeffPreneur];
...
function analysePriseNote([a,b,c,d,e,f,g]){
    if (a==0 ||b == 162){
        totalNOUS = c+e;
        totalEUX = 252 + d+f ;
    }
    ...
    return [totalNOUS,totalEUX,c,d,e,f,coeffPreneur]
}
...
function gains(a){
    ...
    partie = [0,0];
    if ((a[0]>=a[1] & a[6]==1)||(a[0]<=a[1] & a[6]==-1)){
        partie = [a[0],a[1],0];
        if ( a[0]>a[1]){
            gagnants.push(1);
        }
    ...
    else if (a[0]<a[1] & a[6]==1 & a[1] != 252){
        partie = [a[2],162 + a[3]+a[5],0];
        gagnants.push(-1);
    }
    else if(a[0]<a[1] & a[6]==1 & a[1] == 252){
        partie = [a[2],252 + a[3]+a[5],0];
        gagnants.push(-1);

    }
    else if (a[0]>a[1] & a[6]==-1 & a[0] != 252){
        partie = [162 + a[2]+a[4], a[3],0];
        gagnants.push(1)
    }
    else if (a[0]>a[1] & a[6]==-1 & a[0] == 252){
        partie = [252 + a[2]+a[4], a[3],0];
        gagnants.push(1)
    }
    }
    if ((a[0]==a[1])&(a[0]!=0)){
    ...
    if (nb >= 1){
        ...
        if (litige ==0 & (equipeQuiAPrisAvant*equipeQuiAGagne==-1)){
            if ( equipeQuiAPrisAvant == 1){
                ...
                historique[nb]=[belotelitige[0],0,162+belotelitige[1],0];
                gagnants[nb-1]=-1;
            }
            else {
                belotelitige = belote[nb-1];
                historique[nb]=[162+belotelitige[0],0,belotelitige[1],0];
                gagnants[nb-1]=1
            }
        }
...
function equipeBelote(){
    text = '<option value = 0>personne</option><option value = 1>NOUS</option><option value = -1>EUX</option>';
...
function declarationAnnonces(){
    text = "<option value = 0>rien</option><option value = 20>Tierce</option><option value = 50>Cinquante</option><option value = 100>Cent</option><option value = 150>Carré de 9</option><option value = 100>Carré de 10</option><option value = 200>Carré de Valets</option><option value = 100>Carré de Dames</option><option value = 100>Carré de Rois</option><option value = 100>Carré d'As</option>"
    document.getElementById('listedannonces').innerHTML = text;
}

function equipeQUiALAnnonce(){
    text = '<option value = 1>NOUS</option><option value = -1>EUX</option>';
    document.getElementById('equipequialannonce').innerHTML = text;
}
